...
async function assessCapabilityQuality(capabilityId) {
    switch (capabilityId) {
        case "data_point_in_time_replay": {
            const summary = await readJsonIfPresent("data/backtests/polymarket-btc-research-backfill/20260821T093859131Z/summaries/research-backfill-summary.json");
            if (!summary) {
                return {};
            }
        ...
        case "model_proper_scoring": {
            const summary = await readJsonIfPresent("data/backtests/polymarket-btc-barrier/20260820T134713906Z/summaries/barrier-backtest-summary.json");
            if (!summary) {
                return {};
            }
        ...
        case "sim_paper_loops": {
            const performance = await readJsonIfPresent("data/paper-trading/polymarket-btc-milestone/performance-summary.json");
            if (!performance) {
                return {};
            }
        ...
        case "sim_historical_portfolio_replay": {
            const replay = await readJsonIfPresent("data/backtests/polymarket-btc-research-backfill/portfolio-replay/replay-summary.json");
            if (!replay) {
                return {};
            }
        ...
        case "risk_overlap_controls": {
            const research = await readJsonIfPresent("data/paper-trading/polymarket-btc-milestone/research-summary.json");
            if (!research) {
                return {};
            }
