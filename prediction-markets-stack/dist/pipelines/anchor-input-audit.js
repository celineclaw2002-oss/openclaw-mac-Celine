...
export async function runAnchorInputAudit(options = {}) {
    ...
    const [kalshiSummary, coinbaseSummary, deribitSummary, futuresSelected, optionsSelected] = await Promise.all([
        ...
        readOptionalJsonFile(path.join(outputRoot, "anchors", "deribit-btc-anchor-summary.json")),
    ...
    const checkedAtMs = deribitSummary?.observationTimeMs ??
    ...
    await writeFile(path.join(outputRoot, "anchors", "anchor-input-audit.json"), `${JSON.stringify(summary, null, 2)}\n`, "utf8");
    return summary;
}
