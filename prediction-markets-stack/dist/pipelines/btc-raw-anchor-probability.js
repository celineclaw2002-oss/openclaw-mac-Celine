...
export async function runBtcRawAnchorProbability(options = {}) {
    ...
    const [mappingAudit, selectedFutures, selectedOptions, coinbaseSpot, deribitSummary] = await Promise.all([
        readJsonFile(path.join(outputRoot, "anchors", "btc-anchor-mapping-audit.json")),
        readJsonFile(path.join(outputRoot, "anchors", "deribit-btc-futures-selected.json")),
        readJsonFile(path.join(outputRoot, "anchors", "deribit-btc-options-selected.json")),
        readJsonFile(path.join(outputRoot, "anchors", "coinbase-btc-spot-summary.json")),
        readJsonFile(path.join(outputRoot, "anchors", "deribit-btc-anchor-summary.json"))
    ]);
    ...
    for (const row of mappingAudit.rows) {
        ...
        const forwardPrice = future.book?.underlying_price ?? future.book?.mark_price ?? future.book?.mid_price ?? coinbaseSpot.price;
        if (!Number.isFinite(forwardPrice) || forwardPrice <= 0) {
            continue;
        }
        const timeToExpiryYears = (row.evaluationTimestampMs - deribitSummary.recordedAtMs) / (365.25 * 24 * 3_600_000);
        const atmVol = estimateAtmVol(selectedOptions, forwardPrice, row.evaluationTimestampMs);
        const rawProbability = row.marketStructure === "bucket"
            ? computeBucketProbability(row.bucketLowerBound, row.bucketUpperBound, forwardPrice, atmVol, timeToExpiryYears)
            : computeThresholdProbability(row.comparisonOperator, forwardPrice, row.thresholdValue, atmVol, timeToExpiryYears);
    ...
    await writeFile(path.join(outputRoot, "anchors", "btc-raw-anchor-probabilities.json"), `${JSON.stringify(anchors, null, 2)}\n`, "utf8");
    ...
    await writeFile(path.join(outputRoot, "anchors", "btc-raw-anchor-summary.json"), `${JSON.stringify(summary, null, 2)}\n`, "utf8");
    return summary;
}
...
function estimateAtmVol(options, forwardPrice, evaluationTimestampMs) {
    const filtered = options
        .filter((option) => option.expiration_timestamp >= evaluationTimestampMs &&
...
function computeThresholdProbability(operator, forwardPrice, strike, sigma, timeToExpiryYears) {
...
function computeBucketProbability(lowerBound, upperBound, forwardPrice, sigma, timeToExpiryYears) {
