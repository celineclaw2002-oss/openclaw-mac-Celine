...
export async function runPolymarketBtcPortfolioReplay(options = {}) {
    ...
    for (const snapshot of snapshots) {
        ...
        for (const position of openPositions) {
            ...
            const shouldExit = !current || !current.allowEntry || current.ensembleScore < 0.42 || current.netEdgeToEntry < 0.01 || current.horizonDays <= 10;
            if (shouldExit) {
        ...
        const exposureBySleeve = new Map();
        for (const position of openPositions) {
        ...
        const refreshedMarkedValue = openPositions.reduce((sum, position) => sum + (position.markPriceCents ?? position.entryPriceCents) * position.quantity, 0);
        ...
        await writeFile(path.join(outputRoot, "loops", `${snapshot.timestampIso.replaceAll(":", "").replaceAll(".", "")}.json`), `${JSON.stringify(loopSummary, null, 2)}\n`, "utf8");
    }
...
async function ensureReplayableBackfill() {
...
async function resolveLatestBackfillRoot() {
...
async function loadReplaySnapshots(backfillRoot) {
    ...
    const snapshots = await Promise.all(entries.map(async (entry) => JSON.parse(await readFile(path.join(snapshotsRoot, entry), "utf8"))));
