...
export async function runKalshiLiveCapture(options = {}) {
    ...
    const seriesSelectionDiagnostics = [];
    ...
    for (const seriesTicker of targetSeriesTickers) {
        ...
        const tradableCandidates = seriesCandidates.filter((candidate) => isObject(candidate.marketPayload) &&
            isMarketTradableCandidate({
                status: typeof candidate.marketPayload.status === "string" ? candidate.marketPayload.status : null,
                open_time: typeof candidate.marketPayload.open_time === "string" ? candidate.marketPayload.open_time : null
            }, Date.now()));
        ...
        const chosenFamilyKey = seriesTicker === "KXBTC" ? chooseBestTradableFamilyKey(tradableCandidates) : undefined;
    ...
    const uniqueEventTickers = [...new Set(candidateMarkets
    ...
    const [seriesDetails, eventDetails] = await Promise.all([
        Promise.all(uniqueSeriesTickers.map(async (seriesTicker) => {
            const response = await client.getSeries(seriesTicker);
            return [seriesTicker, response.series];
        })),
        Promise.all(uniqueEventTickers.map(async (eventTicker) => {
            const response = await client.getEvent(eventTicker);
            return [eventTicker, response.event];
        }))
    ]);
    ...
    for (const observation of internalObservations.filter((row) => row.executionSafeFlag)) {
        for (const template of ["aggressive_all_legs", "passive_first", "hybrid_edge_tiered"]) {
    ...
    await Promise.all([
        ...
        store.writeSnapshot("summaries/series-fee-metadata.json", [...seriesByTicker.entries()].map(([seriesTicker, series]) => ({
            seriesTicker,
        ...
        store.writeSnapshot("summaries/event-fee-metadata.json", [...eventByTicker.entries()].map(([eventTicker, event]) => ({
            eventTicker,
...
function buildMarketsEndpoint(seriesTicker, limit, cursor) {
...
function isMarketTradableCandidate(market, nowMs) {
...
function scoreCandidateMarket(market, nowMs) {
    const status = market.status?.toLowerCase();
    const openTimeMs = market.open_time ? Date.parse(market.open_time) : undefined;
...
function chooseBestTradableFamilyKey(candidates) {
...
function summarizeSelectedBySeries(candidates) {
...
function summarizeTradableSelectedBySeries(candidates, candidateDiscoveryMarkets) {
    const counts = new Map();
    for (const candidate of candidates) {
        const marketPayload = candidateDiscoveryMarkets.get(candidate.ticker)?.marketPayload;
        if (!isObject(marketPayload) ||
            !isMarketTradableCandidate({
