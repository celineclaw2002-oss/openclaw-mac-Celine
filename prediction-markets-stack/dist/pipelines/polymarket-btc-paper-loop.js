...
export async function runPolymarketBtcPaperLoop(options = {}) {
    ...
    for (const position of portfolio.openPositions) {
        ...
        const positionPolicy = market && anchorProbability !== undefined
...
function computeBarrierHitProbability(spotPrice, barrier, endDate, annualizedVol, nowMs) {
    ...
    const logBarrier = barrier.direction === "up" ? Math.log(barrier.price / spotPrice) : Math.log(spotPrice / barrier.price);
...
function classifyExitReason(signal, exitEdgeThreshold, market, executableExitPrice, policy) {
...
async function resolveLatestBacktestSummaryPath(cwd) {
...
function deriveCandidatePolicy(inputs) {
    ...
    const weakestImprovement = Math.min(barrierImprovement.brierImprovement, barrierImprovement.logLossImprovement, horizonImprovement.brierImprovement, horizonImprovement.logLossImprovement);
    const qualityScore = (barrierImprovement.brierImprovement +
...
function selectNearestSegment(segments, groupType, value) {
...
function computeSegmentImprovement(segment) {
    return {
...
function passesCorrelationControls(inputs) {
    ...
    const sameEventExposureCents = sameEventPositions.reduce((sum, position) => sum + (position.lastMarkPriceCents ?? position.entryPriceCents) * position.quantity, 0) + inputs.candidateCostCents;
    ...
    for (const position of sameDirectionPositions) {
        ...
        if (existingBarrierPrice === undefined ||
...
function computeOverlapAdjustedQuantity(inputs) {
    ...
    const sameEventExposureCents = sameEventPositions.reduce((sum, position) => sum + (position.lastMarkPriceCents ?? position.entryPriceCents) * position.quantity, 0);
    ...
    for (const position of sameDirectionPositions) {
        ...
        const gapRatio = Math.abs(existingBarrierPrice - inputs.candidate.barrier.price) /
...
function buildResearchSnapshot(inputs) {
    ...
    const allowedCandidates = inputs.candidates.filter((candidate) => candidate.policy.allowEntry && Math.abs(candidate.signal) >= candidate.policy.entryEdgeThreshold);
    const eventExposure = new Map();
    const directionExposure = new Map();
    ...
    return {
        ...
        costDiagnostics: {
            expectedEntryCostCents: Math.round(inputs.openPositions.reduce((sum, position) => {
...
function buildAttributionBuckets(openPositions, closedPositions, groupKey) {
    const buckets = new Map();
    const upsert = (bucketId) => {
...
function deriveObservedSpreadCostCents(position) {
...
function inferMarketEndDate(questionText) {
...
async function loadPortfolioState(portfolioRoot, startingCashCents) {
    ...
    try {
        return normalizePortfolioState(JSON.parse(await readFile(target, "utf8")));
    }
    catch {
        return {
...
function normalizePortfolioState(state) {
    return {
        ...state,
...
function normalizeClosedPaperPosition(position) {
    return normalizePaperPosition(position);
}
