...
export async function runBtcMarketReadinessAudit(options = {}) {
    ...
    const [captureSummary, discoveryPages] = await Promise.all([
        ...
        readJsonFile(path.join(outputRoot, "summaries", "discovery-pages.json"))
    ]);
    const captureCompletedAtMs = captureSummary.captureCompletedAtMs ?? Date.now();
    const rowsByEventTicker = new Map();
    ...
    const families = [...rowsByEventTicker.entries()]
    ...
    await writeFile(path.join(outputRoot, "summaries", "btc-market-readiness.json"), `${JSON.stringify(summary, null, 2)}\n`, "utf8");
    return summary;
}
...
function buildFamilyReadinessRow(eventTicker, markets, nowMs) {
