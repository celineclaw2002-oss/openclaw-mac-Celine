...
export async function runInternalConsistencyValidationMatrix(options = {}) {
    ...
    for (const series of eligibility) {
        ...
        const trades = JSON.parse(await readFile(tradesPath, "utf8"));
...
async function discoverSeries(capturesRoot, requiredEdgeType) {
    ...
    for (const captureId of directories) {
        ...
        const observations = await readJsonIfPresent(path.join(captureRoot, "observations", "internal-consistency.json"));
        const edges = await readJsonIfPresent(path.join(captureRoot, "graphs", "edges.json"));
        if (!observations || !edges) {
            continue;
        }
        const edgeSeries = new Map();
        ...
        for (const observation of observations) {
            ...
            const stats = discovered.get(seriesKey) ?? { captures: new Set(), observations: 0 };
...
function summarizeEligibility(seriesKey, stats, gateConfig) {
    const reasons = [];
...
function evaluateWalkForwardGates(walkForward, topPolicy, uniqueEvents, gateConfig) {
    const failed = [];
...
function deriveVerdict(failedGates, cumulativePnlToResolution) {
