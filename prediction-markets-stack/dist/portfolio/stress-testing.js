...
function buildScenario(scenarioId, title, baseShock, inputs, candidateMap) {
    const impacts = inputs.openPositions.map((position) => {
        ...
        const convexityPenalty = position.barrierMultiplier === undefined ? 1 : 1 + Math.min(0.5, Math.abs(position.barrierMultiplier - 1));
