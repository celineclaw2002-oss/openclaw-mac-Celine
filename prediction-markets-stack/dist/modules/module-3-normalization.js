...
export class KalshiNormalizationModule {
    buildFamily(discovery, metadata) {
    ...
    buildContract(discovery, metadata) {
    ...
    buildThreshold(contract, metadata) {
...
function metadataToDiscovery(metadata) {
    return {
...
function inferReferenceVariable(...texts) {
...
function inferContractType(discovery, metadata) {
...
function inferOutcomeType(discovery, metadata) {
    const text = `${discovery.questionText} ${metadata.rulesText}`.toLowerCase();
...
function extractThresholdMetadata(text) {
...
function extractBucketRange(text) {
    const normalized = text.toLowerCase();
...
function calculateSemanticConfidence(discovery, metadata) {
    let score = 0.55;
...
function collectAmbiguityFlags(discovery, metadata) {
    ...
    if (!extractThresholdMetadata(`${discovery.questionText} ${metadata.rulesText}`) &&
...
function buildFamilyNotes(discovery, metadata) {
...
function inferFamilyClassFromText(questionText) {
