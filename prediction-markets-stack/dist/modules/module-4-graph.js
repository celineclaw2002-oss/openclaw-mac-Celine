...
export class DeterministicGraphModule {
    generateComplementEdges(contracts, graphVersion) {
    ...
    generatePartitionEdges(contracts, buckets, graphVersion) {
    ...
    generateThresholdLadderEdges(contracts, thresholds, graphVersion) {
        const contractsById = new Map(contracts.map((contract) => [contract.contractId, contract]));
        const grouped = new Map();
        for (const threshold of thresholds) {
            const operatorFamily = threshold.comparisonOperator === ">" || threshold.comparisonOperator === ">=" ? "upper" : "lower";
        ...
        for (const group of grouped.values()) {
            ...
            for (let index = 0; index < sorted.length - 1; index += 1) {
                ...
                edges.push({
                    ...
                    confidenceScore: Math.min(contractsById.get(left.contractId)?.semanticConfidence ?? 0.5, contractsById.get(right.contractId)?.semanticConfidence ?? 0.5),
                    ruleDependencyScore: 0.9,
                    graphVersion,
                    supportingEvidence: {
    ...
    generateTemporalNestedEdges(contracts, thresholds, graphVersion) {
        const contractsById = new Map(contracts.map((contract) => [contract.contractId, contract]));
        const edges = [];
        const grouped = new Map();
        for (const threshold of thresholds) {
            ...
            for (let index = 0; index < group.length; index += 1) {
                for (let inner = index + 1; inner < group.length; inner += 1) {
                    ...
                    edges.push({
                        ...
                        confidenceScore: Math.min(contractsById.get(earlier.contractId)?.semanticConfidence ?? 0.5, contractsById.get(later.contractId)?.semanticConfidence ?? 0.5),
...
function normalizeQuestionStem(questionText) {
    return questionText
        .toLowerCase()
