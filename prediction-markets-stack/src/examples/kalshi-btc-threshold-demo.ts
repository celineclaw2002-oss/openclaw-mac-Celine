...
export async function runKalshiBtcThresholdDemo(): Promise<DemoSummary> {
  ...
  const discoveryEvents = [thresholdMarket, bucketLow, bucketHigh].map((payload, index) =>
    factory.buildEvent(
      {
        ...captureBase,
  ...
  const metadataEvents = [thresholdMarket, bucketLow, bucketHigh].map((payload, index) =>
    factory.buildEvent(
      {
        ...captureBase,
  ...
  const contracts = discoveries.map((discovery, index) =>
    normalization.buildContract(discovery, metadatas[index]!)
  );
