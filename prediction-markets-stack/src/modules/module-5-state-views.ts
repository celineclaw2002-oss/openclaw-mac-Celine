...
function bestBidFromLevels(levels: Array<[price: number, size: number]> | undefined): number | undefined {
  if (!levels || levels.length === 0) {
    return undefined;
  }
  return levels.reduce<number | undefined>((best, [price]) => {
...
function scoreQuoteQuality(
  bestYesBid: number | undefined,
  bestYesAsk: number | undefined,
  bestNoBid: number | undefined,
  bestNoAsk: number | undefined,
  yesBids: Array<[price: number, size: number]> | undefined,
  noBids: Array<[price: number, size: number]> | undefined
...
function filterExecutionRelevantFlags(flags: string[]): string[] {
  return flags.filter((flag) =>
    !["missing_settlement_source", "missing_primary_rules", "sparse_question_text"].includes(flag)
  );
}
