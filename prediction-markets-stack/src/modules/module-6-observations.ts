...
export class DeterministicObservationModule implements ObservationModule {
  ...
  async buildInternalEdgeObservation(
    ...
    const executionSummary = summarizeExecution(
      allContractIds,
      this.inputs.executionByContractId,
      this.inputs.quotesByContractId
    );
    const qualityFlags = new Set<string>(quoteSummary.derivedFlags);
    if (targetMid === null) {
    ...
    return {
      ...
      executionSafeFlag:
        quoteSummary.averageQuoteQuality >= 0.75 &&
...
function inferEventFamilyId(contractId: string | undefined): string {
  if (!contractId) {
...
export function deriveQuoteMidpoint(quote: ContractQuoteState): number | null {
...
function summarizeQuotes(
  contractIds: string[],
  quotes: Map<string, ContractQuoteState>
): { averageQuoteQuality: number; derivedFlags: string[] } {
  const present = contractIds.map((contractId) => quotes.get(contractId)).filter((quote): quote is ContractQuoteState => quote !== undefined);
  if (present.length === 0) {
    return {
  ...
  const averageQuoteQuality =
    present.reduce((sum, quote) => sum + quote.quoteQualityScore, 0) / present.length;
...
function summarizeExecution(
  contractIds: string[],
  executionStates: Map<string, ContractExecutionState>,
  quotes: Map<string, ContractQuoteState>
): {
  averageExecutionPenalty: number;
  modeledFillProbability: number;
  flags: string[];
} {
  let totalPenalty = 0;
  ...
  for (const contractId of contractIds) {
    ...
    const executionPenalty = Math.max(
      0.1,
      (1 - depthScore) * 1.1 + (1 - flowScore) * 0.6 + (1 - queueScore) * 0.4 + asymmetry * 0.4
    );
...
function extractVisibleDepth(quote: ContractQuoteState | undefined): number {
  if (!quote) {
    return 0;
  }
  const yesDepth = Array.isArray(quote.depthLevels.yesBids)
    ? (quote.depthLevels.yesBids as Array<[number, number]>).reduce((sum, [, size]) => sum + size, 0)
    : 0;
  const noDepth = Array.isArray(quote.depthLevels.noBids)
    ? (quote.depthLevels.noBids as Array<[number, number]>).reduce((sum, [, size]) => sum + size, 0)
    : 0;
...
function summarizeFees(
  contractIds: string[],
  fees: Map<string, ContractFeeState>,
  quotes: Map<string, ContractQuoteState>
): {
  averageFee: number;
  missingFeeContracts: number;
  placeholderFeeContracts: number;
  feeModes: string[];
} {
  let totalFee = 0;
...
function isExecutionBlockingFlag(flag: string): boolean {
  return (
    flag === "missing_quote_state" ||
