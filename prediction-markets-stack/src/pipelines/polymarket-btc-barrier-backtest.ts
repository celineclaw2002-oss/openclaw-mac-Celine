...
interface BacktestObservation {
  ...
  realizedVol: number;
  realizedHit: 0 | 1;
  rawBarrierProbability: number;
  terminalBaselineProbability: number;
}
...
export async function runPolymarketBtcBarrierBacktest(
  options: PolymarketBtcBarrierBacktestOptions = {}
): Promise<PolymarketBtcBarrierBacktestSummary> {
  const outputRoot =
    options.outputRoot ?? path.resolve(process.cwd(), "data", "backtests", "polymarket-btc-barrier", timestampId(new Date()));
  ...
  const overall = {
    ...
    calibratedBarrier: summarizeModel(
      "calibrated_barrier",
      calibratedTest.map((row) => row.calibratedBarrierProbability),
      calibratedTest.map((row) => row.realizedHit)
    ),
    terminalBaseline: summarizeModel(
      "terminal_baseline",
      test.map((row) => row.terminalBaselineProbability),
      test.map((row) => row.realizedHit)
    )
  };

  const segmented = buildSegmentedMetrics(calibratedTest);
  const summary: PolymarketBtcBarrierBacktestSummary = {
    outputRoot,
    checkedAtIso: new Date().toISOString(),
    sourceNote:
      "Synthetic historical BTC milestone backtest grounded in first-passage / barrier-hit modeling and proper scoring. This evaluates model quality, not venue execution or market-specific edge.",
    startIso,
    endIso,
...
async function fetchDailyCandles(startIso: string, endIso: string): Promise<CoinbaseDailyCandleRecord[]> {
...
function buildObservations(
  candles: CoinbaseDailyCandleRecord[],
  lookbackDays: number,
  horizonDays: number[],
  barrierMultipliers: number[]
): BacktestObservation[] {
...
function fitIsotonicCalibration(probabilities: number[], outcomes: number[]): IsotonicBlock[] {
  const rows = probabilities
    .map((probability, index) => ({ x: clamp01(probability), y: outcomes[index] ?? 0, weight: 1 }))
    .sort((left, right) => left.x - right.x);
  const blocks: Array<{ lowerX: number; upperX: number; sumY: number; weight: number }> = [];
...
function predictIsotonic(blocks: IsotonicBlock[], probability: number): number {
...
function summarizeModel(modelId: string, predictions: number[], outcomes: number[]): ModelMetrics {
  return {
    modelId,
...
function buildSegmentedMetrics(
  rows: Array<BacktestObservation & { calibratedBarrierProbability: number }>
): SegmentMetrics[] {
  ...
  return [...groups.entries()].map(([key, groupRows]) => {
    ...
    const groupType = key.slice(0, separatorIndex) as SegmentMetrics["groupType"];
    ...
    return {
      ...
      calibratedBarrier: summarizeModel(
        "calibrated_barrier",
        groupRows.map((row) => row.calibratedBarrierProbability),
        outcomes
      ),
...
function appendGroup<T>(groups: Map<string, T[]>, key: string, value: T): void {
...
function buildVerdict(overall: PolymarketBtcBarrierBacktestSummary["overall"]): string {
...
function computeBarrierHitProbability(spot: number, barrier: number, sigma: number, timeYears: number): number {
  if (spot >= barrier) {
    return 1;
  }
...
function computeTerminalProbability(spot: number, barrier: number, sigma: number, timeYears: number): number {
