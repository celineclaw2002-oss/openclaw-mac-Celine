...
import {
  DeribitBookSummaryRecord,
  DeribitHttpClient,
  DeribitInstrumentRecord
} from "../runtime/deribit-api.js";
...
export async function runDeribitBtcAnchorCapture(
  options: DeribitBtcAnchorCaptureOptions = {}
): Promise<DeribitBtcAnchorCaptureSummary> {
  ...
  const selectedOptions = selectRepresentativeOptions(
    optionInstruments,
    optionBooks,
    referenceSpotPrice,
    nearExpiryCount,
  ...
  const observationTimeMs =
    latestSelectedBookTimestamp(selectedFutures, selectedOptions) ??
  ...
  const summary: DeribitBtcAnchorCaptureSummary = {
    outputRoot,
    currency,
    observationTimeMs,
    recordedAtMs,
...
function latestSelectedBookTimestamp(
  futures: Array<DeribitInstrumentRecord & { book?: DeribitBookSummaryRecord }>,
  options: Array<DeribitInstrumentRecord & { book?: DeribitBookSummaryRecord }>
): number | undefined {
...
function latestBookTimestamp(books: Array<DeribitBookSummaryRecord | undefined>): number | undefined {
...
async function readLatestCoinbaseSpot(
  outputRoot: string
): Promise<{ price?: number; observationTimeMs?: number; recordedAtMs?: number; payloadRef?: string } | undefined> {
...
function selectNearestExpiries(
  instruments: DeribitInstrumentRecord[],
  books: DeribitBookSummaryRecord[],
  nearExpiryCount: number
): Array<DeribitInstrumentRecord & { book?: DeribitBookSummaryRecord }> {
...
function selectRepresentativeOptions(
  instruments: DeribitInstrumentRecord[],
  books: DeribitBookSummaryRecord[],
  referenceSpotPrice: number | undefined,
  nearExpiryCount: number,
  strikeDistanceFraction: number
): Array<DeribitInstrumentRecord & { book?: DeribitBookSummaryRecord }> {
  const booksByInstrument = new Map(books.map((book) => [book.instrument_name, book]));
  const selectedExpiries = Array.from(
    new Set(instruments.map((instrument) => instrument.expiration_timestamp))
  )
    .sort((left, right) => left - right)
    .slice(0, nearExpiryCount);
